...
public class UniWebViewInterface
{
	...
	{
		correctPlatform = (Application.platform == RuntimePlatform.Android);
		GameObject gameObject = new GameObject("UniWebViewAndroidStaticListener");
		gameObject.AddComponent<UniWebViewAndroidStaticListener>();
	...
	{
		CheckPlatform();
		return plugin.CallStatic<string>("getUrl", new object[1]
		{
			name
		});
	}
	...
	{
		CheckPlatform();
		return plugin.CallStatic<bool>("show", new object[5]
		{
			name,
			fade,
			edge,
	...
	{
		CheckPlatform();
		return plugin.CallStatic<bool>("hide", new object[5]
		{
			name,
			fade,
			edge,
	...
	{
		CheckPlatform();
		return plugin.CallStatic<bool>("animateTo", new object[8]
		{
			name,
			x,
			y,
	...
	{
		CheckPlatform();
		return plugin.CallStatic<string>("getUserAgent", new object[1]
		{
			name
		});
	}
	...
	{
		CheckPlatform();
		return plugin.CallStatic<string>("getCookie", new object[2]
		{
			url,
			key
		});
	...
	{
		CheckPlatform();
		return plugin.CallStatic<float>("getWebViewAlpha", new object[1]
		{
			name
		});
	}
	...
	{
		CheckPlatform();
		return plugin.CallStatic<bool>("canGoBack", new object[1]
		{
			name
		});
	}
	...
	{
		CheckPlatform();
		return plugin.CallStatic<bool>("canGoForward", new object[1]
		{
			name
		});
	}
