...
public class AIMove1015 : AIMoveBase
{
	...
	protected override void OnInitBase()
	{
		...
		List<Grid.NodeItem> list = GameLogic.Release.Path.FindingPath(m_Entity.position, target.position);
		if (list.Count > 0)
		{
			Vector3 worldPosition = GameLogic.Release.MapCreatorCtrl.GetWorldPosition(list[0].x, list[0].y);
